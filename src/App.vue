	...
	<div id="app" class="container-fluid mb-10">
		<vue-bootstrap4-table :classes="classes"
			...
			<template slot="id" slot-scope="props">
				<a href="javascript:;" class="float-left" @click.stop="$refs.vbt.toggleSubrow(props.row)">
					<i :class="['fa', { 'fa-caret-right': !isExpanded(props.row), 'fa-caret-down': isExpanded(props.row) }]"></i>
				</a>
				{{ props.row.id }}
			</template>
			...
			<template slot="pagination-previous-button">
				Previous
			</template>
			<!-- <template slot="pagination-next-button">
...
<script>
	...
	export default {
		...
		methods: {
			...
			isExpanded(row){
				return this.$refs.vbt.expanded_subrows.indexOf(row.vbt_id) > -1
			}
		},
		components: {
			VueBootstrap4Table,
		...
		computed:{
			selected_items(){
				return (this.$refs.vbt ? this.$refs.vbt.selected_items : [])
			}
		},
		mounted() {
			let self = this;
